          ...
          if (this.file.data || (!this.preload && this.url)) {
            ...
            switch (this.file.metadata.type) {
              case "html":
                ...
                childContent = "<iframe id='idIframe' onload='window.App.iframeLoaded()' style='width:100%;height:100%;' srcdoc='" + this.file.data.replace("'", "\'") + "'></iframe>";
            ...
            // this.viewer.addChild(new ContentPane({
            ...
            this.viewer.addChild(c);
            setTimeout(function() {
              _self.viewer.resize()
            }, 5000);
          } else {
            this.viewer.addChild(new ContentPane({
              content: '<pre style="font-size:.8em; background-color:#ffffff;">Loading file preview.  Content will appear here when available.  Wait time is usually less than 10 seconds.</pre>',
              region: "center"
            }));
          }
        } else {
